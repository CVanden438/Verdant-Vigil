...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    public static string[] FindTagNames()
    {
        ...
        return active != null
            ? active.GetTagNames()
            : new string[1] { "There is no AstarPath component in the scene" };
    }
            ...
            if (graphs[i] != null && graphs[i].drawGizmos)
            {
                graphs[i].GetNodes(node =>
                {
                    if (
                        ...
                        && (
                            ignoreSearchTree
                            || Pathfinding.Util.GraphGizmoHelper.InSearchTree(
                                node,
                                debugPathData,
                                debugPathID
                            )
    ...
    IEnumerator DelayedGraphUpdate()
    {
        ...
        yield return new WaitForSeconds(
            graphUpdateBatchingInterval - (Time.realtimeSinceStartup - lastGraphUpdate)
        );
        QueueGraphUpdates();
    ...
    public void UpdateGraphs(GraphUpdateObject ob)
    {
        ...
        if (
            batchGraphUpdates
            && Time.realtimeSinceStartup - lastGraphUpdate < graphUpdateBatchingInterval
        )
        {
    ...
    {
        AstarProfiler.InitializeFastProfile(
            new string[14]
            {
                "Prepare", //0
                "Initialize", //1
    ...
    public IEnumerable<Progress> ScanAsync(NavGraph[] graphsToScan = null)
    {
        ...
        for (int i = 0; i < graphsToScan.Length; i++)
        {
            ...
            var progressDescriptionPrefix =
                "Scanning graph " + (i + 1) + " of " + graphsToScan.Length + " - ";
    ...
    public static void BlockUntilCalculated(Path path)
    {
        if (active == null)
            throw new System.Exception(
                "Pathfinding is not correctly initialized in this scene (yet?). "
        ...
        if (path.PipelineState < PathState.ReturnQueue)
        {
            ...
            else
            {
                ...
                {
                    if (
                        active.pathProcessor.queue.IsEmpty
                        && path.PipelineState != PathState.Processing
                    )
                    {
                        waitForPathDepth--;
                        throw new System.Exception(
                            "Critical error. Path Queue is empty but the path state is '"
                                + path.PipelineState
                                + "'"
                        );
                    }
    ...
    public static void StartPath(Path path, bool pushToFront = false)
    {
        ...
        {
            Debug.LogError(
                "There is no AstarPath object in the scene or it has not been initialized yet"
            );
            return;
        }
    ...
    public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
    {
        ...
        if (
            ...
            && !constraint.Suitable(nearestNode.node)
        )
        {
        ...
        if (
            !constraint.Suitable(nearestNode.node)
            || (
                constraint.constrainDistance
                && (nearestNode.clampedPosition - position).sqrMagnitude > maxNearestNodeDistanceSqr
            )
        )
        {
            return new NNInfo();
    ...
    public GraphNode GetNearest(Ray ray)
    {
        ...
        for (int i = 0; i < graphs.Length; i++)
        {
            ...
            graph.GetNodes(node =>
            {
                ...
                Vector3 p =
                    lineOrigin + (Vector3.Dot(pos - lineOrigin, lineDirection) * lineDirection);
