...
namespace Pathfinding
{
    ...
    public abstract class AIBase : VersionedMonoBehaviour
    {
        ...
        /// </summary>
        [System.Obsolete(
            "Use the destination property or the AIDestinationSetter component instead"
        )]
        ...
        public Vector3 velocity
        {
            get
            {
                return lastDeltaTime > 0.000001f
                    ? (prevPosition1 - prevPosition2) / lastDeltaTime
                    : Vector3.zero;
            }
        }
        ...
        protected virtual bool shouldRecalculatePath
        {
            get
            {
                return !waitingForPathCalculation
                    && autoRepath.ShouldRecalculatePath((IAstarAI)this);
            }
        }
        ...
        protected abstract void MovementUpdateInternal(
            float deltaTime,
            out Vector3 nextPosition,
            out Quaternion nextRotation
        );
        ...
        protected void ApplyGravity(float deltaTime)
        {
            ...
            if (usingGravity)
            {
                ...
                velocity2D += movementPlane.ToPlane(
                    deltaTime * (float.IsNaN(gravity.x) ? Physics.gravity : gravity),
                    out verticalGravity
                );
        ...
        protected Vector2 CalculateDeltaToMoveThisFrame(
            Vector2 position,
        ...
        protected Quaternion SimulateRotationTowards(Vector2 direction, float maxDegrees)
        {
            if (direction != Vector2.zero)
            {
                Quaternion targetRotation = Quaternion.LookRotation(
                    movementPlane.ToWorld(direction, 0),
        ...
        void FinalizePosition(Vector3 nextPosition)
        {
            ...
            if ((positionDirty1 || positionDirty2) && updatePosition)
            {
                ...
                if (rigid != null)
                    rigid.MovePosition(currentPosition);
                else if (rigid2D != null)
        ...
        protected Vector3 RaycastPosition(Vector3 position, float lastElevation)
        {
            ...
            float rayLength =
                tr.localScale.y * height * 0.5f + Mathf.Max(0, lastElevation - elevation);
        ...
        protected virtual void OnDrawGizmos()
        {
            ...
            {
                Draw.Gizmos.Cylinder(
                    position,
                    rotation * Vector3.up,
                    tr.localScale.y * height,
                    radius * tr.localScale.x,
                    color
                );
            }
